...
import org.lrdm.topologies.strategies.StarTopologyStrategy;
 ...
 *
 */
public class ExampleSimulationStar {
	public static void main(String[] args) {
		System.setProperty("java.util.logging.SimpleFormatter.format",
				"[%1$tF %1$tT] [%4$-7s] %5$s %n");
		TimedRDMSim sim = new TimedRDMSim();
		sim.initialize(new StarTopologyStrategy());
		...
		effector.setStrategy(new StarTopologyStrategy(), 40);
		effector.setStrategy(new FullyConnectedTopology(), 60);
		effector.setStrategy(new StarTopologyStrategy(), 80);
